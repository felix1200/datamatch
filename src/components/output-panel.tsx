...
export function OutputPanel({ formulas, sourceSheet, targetSheet, config, lookupColumn, returnColumn }: OutputPanelProps) {
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null);
  const [activeTab, setActiveTab] = useState('formula');

  const matchResults = useMemo(() => {
    if (!sourceSheet || !targetSheet || !lookupColumn || !returnColumn) return null;
    return executeVlookup(sourceSheet.rows, targetSheet, lookupColumn, returnColumn, config.matchMode);
  }, [sourceSheet, targetSheet, lookupColumn, returnColumn, config.matchMode]);
  ...
  const handleDownload = useCallback(() => {
    ...
    const exportData = sourceSheet.rows.map((row, i) => ({
      ...row,
      'VLOOKUP Result': matchResults[i]?.value ?? '#N/A',
    }));

    generateExcelFile(exportData, 'vlookup_result.xlsx');
  }, [sourceSheet, matchResults]);

  if (formulas.length === 0) {
    return (
      <div className="rounded-xl border border-border bg-muted/30 p-8 text-center">
        <p className="text-sm text-muted-foreground">配置参数后将自动生成 VLOOKUP 公式</p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 text-sm">
          <span className="text-muted-foreground">
            共生成 <span className="font-semibold text-foreground">{formulas.length}</span> 条公式
          </span>
          {matchResults && (
            <span className="text-muted-foreground">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="formula" className="space-y-3">
          ...
          <div className="rounded-xl border border-border bg-slate-950 overflow-hidden max-h-80 overflow-y-auto">
            <div className="p-4 space-y-1 font-mono text-xs">
              {formulas.map((formula, i) => (
                ...
                >
                  <span className="text-slate-500 w-8 text-right shrink-0 select-none">
                    {i + 1}
                  </span>
                  <code className="text-emerald-400 flex-1 break-all">{formula}</code>
                  <button
                    onClick={() => handleCopy(formula, i)}
                ...
                <table className="w-full text-xs">
                  <thead className="bg-muted/80 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left font-medium text-emerald-700 border-b border-border whitespace-nowrap">
                        {returnColumn}
                    ...
                    {matchResults.slice(0, 200).map((result, i) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-muted/20'}>
                        ...
                        <td className="px-3 py-1.5 text-foreground border-r border-border whitespace-nowrap max-w-48 truncate">
                          {sourceSheet.rows[i]?.[lookupColumn] !== null && sourceSheet.rows[i]?.[lookupColumn] !== undefined
                            ? String(sourceSheet.rows[i][lookupColumn])
                            : ''}
                        </td>
                        <td
                          className={`px-3 py-1.5 border-border whitespace-nowrap max-w-48 truncate font-mono ${
                            result.matched
                              ? 'text-emerald-700 bg-emerald-50/50'
                              : 'text-red-600 bg-red-50/50'
                          }`}
                        >
                          {result.matched ? String(result.value) : '#N/A'}
                        </td>
                      </tr>
                    ))}
                  </tbody>
